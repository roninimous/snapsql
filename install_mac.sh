...
# SnapsQL Automated Installer for macOS
...
echo -e "${PURPLE}[+] Starting SnapsQL Installation for macOS...${NC}"

# Check for Docker
if ! command -v docker &> /dev/null; then
    echo -e "${PURPLE}[ERROR] Docker is not installed. Please install Docker Desktop and try again.${NC}"
    exit 1
fi
...
# On macOS, we generally don't need root, but we need to ensure the directories exist and are writable
...
# Setting 777 permissions to avoid any bind-mount permission issues with Docker containers
...
if command_exists docker-compose; then
    ...
    echo -e "${PURPLE}[ERROR] Docker Compose is not installed. Please install Docker Desktop and try again.${NC}"
    exit 1
fi
